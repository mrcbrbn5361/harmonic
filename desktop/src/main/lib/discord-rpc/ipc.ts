...
export default class IPCClient extends EventEmitter {
  ...
  constructor() { super(); this.createSocket(); }
  ...
  public connect(ipcPath: string) {
    if (this.socket.destroyed) this.createSocket();
  ...
  public close() {
    if (!this.socket.destroyed && !this.socket.closed) {
      try { this.send({}, OPCode.CLOSE); } catch { /* ignore */ }
      this.socket.end();
    }
  }
  public destroy() {
    this.removeAllListeners();
    if (!this.socket.destroyed) { this.socket.removeAllListeners(); this.socket.destroy(); }
  }
}
